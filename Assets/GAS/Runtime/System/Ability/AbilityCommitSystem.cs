    ...
    public partial struct AbilityCommitSystem : ISystem
    {
        ...
        private struct AbilityCatalogCommitJob : IJobChunk
        {
                ...
                out AbilityStateComponent nextRuntime)
            {
                ...
                if (!GASRuntimeDefinitionResolver.TryBuildAbilityActivationPlan(
                    ref catalog,
                    state.Code,
                    state.Owner,
                    target,
                ...
                ref readonly var abilityDefinition = ref GASDefinitionCatalogLookup.GetAbility(ref catalog, plan.AbilityDefinitionIndex);
                if (abilityDefinition.RequirementCount > 0)
                {
                    var ownerTags = TagMaskLookup.HasComponent(state.Owner)
                        ? TagMaskLookup[state.Owner]
                        : default;
                    if (!GASRuntimeRequirementEvaluator.EvaluateAbilityRequirements(
                        ref catalog,
            ...
            {
                if (!GASRuntimeDefinitionResolver.TryBuildGECommandSeed(
                        ref catalog,
                        in plan,
                        seedKind,
