...
namespace GAS.Runtime
{
    ...
    [BurstCompile]
    public partial struct OwnerLocalInstantCommandFlushSystem : ISystem
    {
        private EntityQuery _ownerInstantCommandQuery;
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var records = new NativeList<OwnerLocalInstantCommandRecord>(1, Allocator.TempJob);
            var payloads = new NativeList<GESetByCallerValueBuffer>(1, Allocator.TempJob);
            var collectHandle = new CollectOwnerLocalInstantCommandsJob
            {
            ...
            var flushHandle = new FlushOwnerLocalInstantCommandsJob
            {
                StreamEntity = streamEntity,
                StreamLookup = SystemAPI.GetComponentLookup<GEEffectCommandStreamComponent>(isReadOnly: false),
                StreamCommandLookup = SystemAPI.GetBufferLookup<GEEffectCommandBuffer>(isReadOnly: false),
                StreamSetByCallerLookup = SystemAPI.GetBufferLookup<GESetByCallerValueBuffer>(isReadOnly: false),
                Records = records,
                Payloads = payloads,
            }.Schedule(collectHandle);

            var disposeRecordsHandle = records.Dispose(flushHandle);
            state.Dependency = payloads.Dispose(disposeRecordsHandle);
        }

        private struct OwnerLocalInstantCommandRecord
        {
            public Entity Owner;
        ...
        [BurstCompile]
        private struct CollectOwnerLocalInstantCommandsJob : IJobChunk
        {
            ...
            public NativeList<OwnerLocalInstantCommandRecord> Records;
                ...
                in v128 chunkEnabledMask)
            {
                ...
                while (enumerator.NextEntityIndex(out var entityIndex))
                {
                    ...
                    for (var commandIndex = 0; commandIndex < ownerCommands.Length; commandIndex++)
                    {
                        ...
                        Records.Add(new OwnerLocalInstantCommandRecord
                        {
                            Owner = owner,
        ...
        [BurstCompile]
        private struct FlushOwnerLocalInstantCommandsJob : IJob
        {
            public Entity StreamEntity;
            public ComponentLookup<GEEffectCommandStreamComponent> StreamLookup;
            public BufferLookup<GEEffectCommandBuffer> StreamCommandLookup;
            ...
            public void Execute()
            {
                if (Records.Length == 0
                    ...
                    || !StreamLookup.HasComponent(StreamEntity)
                    || !StreamCommandLookup.HasBuffer(StreamEntity)
                    || !StreamSetByCallerLookup.HasBuffer(StreamEntity))
                {
                    return;
                }

                Records.Sort(new OwnerLocalInstantCommandRecordComparer());
                var streamCommands = StreamCommandLookup[StreamEntity];
                var streamSetByCallerValues = StreamSetByCallerLookup[StreamEntity];
                for (var i = 0; i < Records.Length; i++)
                {
                    var record = Records[i];
                    var command = record.Command;
                    var setByCallerStart = streamSetByCallerValues.Length;
                    ...
                    command.SetByCallerStart = setByCallerCount > 0 ? setByCallerStart : 0;
                    command.SetByCallerCount = setByCallerCount;
                    streamCommands.Add(command);
                }
            }
        }

        private struct OwnerLocalInstantCommandRecordComparer : IComparer<OwnerLocalInstantCommandRecord>
        {
            public int Compare(OwnerLocalInstantCommandRecord x, OwnerLocalInstantCommandRecord y)
            {
