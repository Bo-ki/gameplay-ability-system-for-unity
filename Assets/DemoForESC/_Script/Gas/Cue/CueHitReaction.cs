...
namespace DemoForESC._Script.Gas.Cue
{
    ...
    ///   - 也可在 Timeline 的 PlayCue Action 中使用
    ///
    /// 实现原理：
    ...
    public class CueHitReaction : GameplayCueBase<XParamFloat>
    {
        ...
        private Vector3 ComputeHitDirectionXZ()
        {
            ...
            // Cue来源是 GameplayEffect 时，从 GE 的 CEffectContext.SourceAsc 获取攻击者
            if (_sourceType == CueSourceType.GameplayEffect && _sourceEntity != Entity.Null)
            {
                if (EntityManager.Exists(_sourceEntity) && EntityManager.HasComponent<CEffectContext>(_sourceEntity))
                {
                    var context = EntityManager.GetComponentData<CEffectContext>(_sourceEntity);
                    var sourceGo = EntityHelper.GetGameObjectFromEntity(context.SourceAsc);
                    if (sourceGo != null)
                    {
                        attackerPos = sourceGo.transform.position;
                        hasAttackerPos = true;
                    }
                }
            }
