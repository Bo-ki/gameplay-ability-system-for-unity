...
| Fan-in store | `NativeStream` + `SegmentCounts` + `MergeEffectCommandFanInJob` + sort key | segment budget、capacity / spill、merge cost、battle hash、reselect trigger | singleton `DynamicBuffer` 作为 scale-ready 总线；parallel job 共享写单个 counter |
