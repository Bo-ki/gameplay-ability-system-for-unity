...
目标态不以目录或 facade 名称判断架构优劣，而以每个 Module 的 Interface 是否足够窄、Implementation 是否能隐藏足够多的 ECS 复杂度来判断。理想 EX-GAS 2.0 至少拆成下列 owner：

| Module | 目标 Interface | Implementation owner | 必须隐藏的复杂度 | 禁止泄露 |
|---|---|---|---|---|
| `RuntimeSession` | install / dispose / fixed tick / timing result | Runtime Boundary bootstrap implementation | World 创建、SystemGroup 注册、catalog session、dependency drain、fixed-step policy | `World`、`EntityManager`、SystemGroup 实例 |
| `CommandPort` | `RequestActivateAbility`、`RequestApplyGameplayEffect`、`RequestDestroyAsc` | Boundary command writer + owner-local command buffer | sequence、pending marker、target key resolve、validation reason、write capacity | raw `Entity`、可写 `DynamicBuffer`、同步执行语义 |
| `SnapshotReadModel` | immutable snapshot by key / version | BoundaryProjection snapshot/ring buffer owner | live ECS 读取、copy cost、stale policy、version 推进、snapshot capacity | live `DynamicBuffer`、`EntityQuery`、getter 回读 Core |
| `GASFrameKernel` | command/spec/delta/fact records | hand-written Runtime Core lane systems/jobs | query owner、lookup refresh、allocator、dependency、Burst job、deterministic ordering | OOP manager、managed config、hidden generated lifecycle |
| `EffectFanInStore` | sorted command/spec/delta/fact ranges | NativeStream / owner-local range / compact buffer owner | producer segment、merge cost、sort key、spill、reselect trigger | singleton DynamicBuffer 作为默认 scale-ready fan-in |
| `ActiveEffectStore` | owner-local active slot / lifecycle state | hand-written Core store lane + cleanup lane | slot capacity、stack/period/duration state、cleanup record、cache integrity | helper static method 作为 lifecycle owner |
| `StructuralCommit` | structural intent + playback evidence | `GASStructuralCommitSystemGroup` + ECB systems | ECB sort key、route、bulk query、recorded vs required playback、Journaling/Profiler diff | scattered create/destroy/add/remove |
| `DiagnosticsSink` | structured evidence snapshot | Boundary diagnostics / debugger gather owner | counter schema、official capture state、TopN、derived export source、overhead split | 字符串日志作为唯一 evidence |
| `GeneratedDefinitionGlue` | code -> index -> immutable plan / evaluator | Definition & Generation artifact owner | Luban row projection、Blob schema、lookup table、pure evaluator、validation report | `ISystem`、`OnUpdate`、query、ECB、NativeContainer owner |
| `PresentationBridge` | outbox marker / cue request / render log | Runtime Boundary presentation owner | resource binding、managed side effect、headless marker、copy/flush cost | Cue / UI / VFX / SFX 反向决定 gameplay |

Deep Module 的验收标准是：调用者只理解业务 intent、snapshot、evidence 或 immutable definition；query、allocator、dependency、capacity、merge、structural playback 和 diagnostics source 都停留在 implementation owner 内，并能被 Debugger evidence 归因。
...
这组代码骨架应被视为一个完整框架设计样例，而不是五段可随意摘取的片段。它表达的设计意图是：OOP Shell 只负责把业务意图交给 Boundary；Boundary 只写 command record 或导出 snapshot；Runtime Core 只处理 ECS 数据流；Debugger 只消费 facts / counters；SourceGenerator 只提供不可变定义和纯解析函数。代码骨架中的 `EntityManager` 只能出现在 Boundary implementation 或 bootstrap owner 内，不能被提升为外部业务 interface，也不能被 SourceGenerator 封装成新的 runtime 中间层。
