...
这组代码骨架的重点不是“接口更多”，而是每个 interface 后面隐藏了足够多的实现复杂度，并且这些复杂度有明确 owner。目标态的合理性可以按 deletion test 阅读：删除某个 Module 时，复杂度应只回到它自己的 owner，而不是散落到 Shell、generated artifact、Debugger 和 Demo adapter。

| 目标态 Module | 代码中隐藏的复杂度 | 为什么比聚合 Shell 更合理 |
|---|---|---|
| `RuntimeSession` | World / SystemGroup / catalog lifetime / fixed tick lifecycle | Shell 只得到 session id 和 tick result；bootstrap、dispose、timing split 不再泄漏给 UI / AI / Demo |
| `RunnerSync` | dependency drain reason、measurement fence、runner budget | dependency drain 不再是 command / snapshot API 的隐式副作用，性能归因可以单独审计 |
| `DefinitionCatalogLifetime` | Blob install、schema / content hash、dispose token | 配置生命周期和 gameplay command 分离；Runtime Core hot path 只消费 immutable catalog |
| `CommandPort` | opaque handle resolve、request validation、sequence、owner-local append | 外部只能提交 intent，无法同步读取 live ECS 或把 command write 当 gameplay 结算 |
| `SnapshotReadModel` | BoundaryProjection、snapshot version、cursor、staleness | 读取变成 immutable snapshot 消费，不再要求业务层理解 live buffer / query |
| `GASFrameKernel` | query owner、type handle、lookup refresh、job dependency、frame allocator | Gameplay 权威只存在于 Core ECS data / Job chain，OOP 不再是中间层 |
| `EffectFanInStore` | `NativeStream` segment、sort key、deterministic merge、capacity / spill evidence | transient fan-in 不再依赖全局 singleton 总线，scale gate 能基于 merge cost 和 pressure 判定 |
| `ActiveEffectStore` | owner-local slot、period / stack / granted state、cleanup intent | 跨帧 GE 生命周期集中在 store owner，而不是散落到 helper、generated lifecycle 和 command stream |
| `StructuralCommit` | structural intent、ECB playback phase、official evidence route | create / destroy / add / remove 的来源和 phase 可证明，不依赖隐式 `EntityManager` 写入 |
| `DiagnosticsSink` | runtime counter、official capture state、validation evidence、derived export source | Debugger 能定位热点，但不能反向驱动 simulation，也不能用字符串日志替代 evidence |
| `GeneratedDefinitionGlue` | code -> index lookup、Blob schema、pure evaluator、validation artifact | SourceGenerator 输出可 Burst / unmanaged 消费的 glue，不拥有 lifecycle、query 或 ECB owner |
| `PresentationBridge` | presentation outbox、cue marker、resource binding key | GameObject / VFX / SFX 留在 Boundary，不能污染 Core gameplay write |

这也是目标态拒绝“一个更大的 OOP facade”或“一个更大的 generated runtime”的原因：如果接口返回 ECS handle、隐藏 query、隐藏 ECB、隐藏 generated lifecycle 或隐藏 fan-in capacity，调用者仍然必须理解 implementation，Module 就不够深。目标态要求接口只说业务词汇和 evidence 词汇，复杂度集中在可测试、可计时、可替换的 owner 内。
