...
`CGameplayEventBus` 是当前实现名和过渡事实流容器；目标态命名应向 `GameplayFactStream`、`GameplayFactReadCursor`、`PresentationOutboxBridge` 收敛。
...
6. Presentation summary 能输出 weak resource load / release / missing marker，即使当前只用日志占位。
...
3. 当前 Demo 中 UI 更新只消费事件、与 ECS 解耦的业务信号来自 `../历史方案参考/方案12.md:713-752`。
4. 方案14 的 GASEventBus 示例位于 `../历史方案参考/方案14.md:367-426`，当前只吸收“ECS -> 表现观察”信号，不吸收托管 EventBus 作为实时 gameplay 路由。
