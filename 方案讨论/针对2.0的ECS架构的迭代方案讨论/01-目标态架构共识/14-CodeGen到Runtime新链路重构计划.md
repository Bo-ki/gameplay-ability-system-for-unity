...
- `StaticLookupPhase` 生成的 lookup 已带 `OwnsMemory` 契约，调用方可区分拥有型与借用型 `NativeArray` / `BlobAssetReference` 生命周期；它是迁移期 per-domain lookup，目标态 Runtime hot path 应优先走 `GASDefinitionCatalogBlob` + index/ref readonly 访问。
...
- GASDefinitionCatalogBlob 聚合生成、catalog singleton bootstrap、`AbilityCode -> AbilityDefinitionIndex -> ref readonly AbilityDefinitionBlob -> GameplayEffectDefinitionIndex` Runtime 消费闭环仍待实现；当前 per-definition `GASGeneratedDefinitionBlobComponent<T>` 只允许作为 baking output / bootstrap 收集入口，不作为 hot path lookup。
- `RuntimeDefinitionGluePhase` 仍待实现：需要生成 `GASGeneratedRuntimeDefinitionResolver`、`GASGeneratedRequirementEvaluator`、`GASGeneratedMagnitudeEvaluator`、`GASGeneratedTargetRuleTable`，并证明 `AbilityDefinitionIndex -> AbilityActivationPlanRecord -> GECommandSeedRecord -> ResolvedModifierRecord` 全程无 managed row / JSON / `Dictionary`。
...
3. 若完整链路通过，下一步转入 GAS Runtime 消费 generated `GASDefinitionCatalogBlob` / lookup / Runtime definition glue，而不是继续补 AutoChessDemo 旧业务链路。
