...
当前实现状态：`Assets/AutoChessDemo` 已先按破坏性重构落地业务导向的 2v2 最小 Runtime Core 验证链，当前可执行链路是 `GameRoom -> Battle -> Integration/GasCore -> Battle/Ecs -> Observation/Result -> Presentation`；旧 `AutoBattle` / `HeadlessAutoChess*` 代码和命名不再作为过渡兼容对象。10B 仍是完整业务案例目标态；它不能被当前 2v2 最小切片替代，也不能要求当前切片保留旧 OOP config registry / scenario 巨类。
