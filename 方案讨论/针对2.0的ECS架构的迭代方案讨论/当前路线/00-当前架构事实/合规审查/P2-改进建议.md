...
**最近审查**: 2026-05-26 | **状态**: 8 活跃（AC/AD/AE 新增）
...
### 缺陷 AC: Luban 配置目录结构与 Spec 11 不一致 **[2026-05-26 新增]**

**严重程度**: P2

**代码证据**: Spec 11 定义 `Datas/AutoChessDemo/` 子目录，当前 8 张 Excel 表直接放在 `EX_GAS_Config/ProjectConfigTable/exgas_config/Datas/` 下，缺少 `AutoChessDemo/` 层级。不影响运行时行为，但违反 Spec 定义的目标目录结构。

### 缺陷 AD: 生成物未按 Spec 11 拆分为独立文件 **[2026-05-26 新增]**

**严重程度**: P2

**代码证据**: Spec 11 定义 6 个独立 `.g.cs` 文件（Ids/Lookups/BlobBuilders/ScenarioBuildPlan/ValidationExpectations/ConfigDiagnostics），当前全部嵌入单体 `HeadlessAutoChessGeneratedDefinitionRows.cs`（~2500行）。影响编译依赖最小化和 Editor/CI 诊断独立分发。

### 缺陷 AE: `GASGeneratedDefinitionBakingSystem` 使用 SystemBase 而非 ISystem **[2026-05-26 新增]**

**严重程度**: P2

**代码证据**: `GASGeneratedDefinitionBakingSystem.cs:22` — `public partial class GASGeneratedDefinitionBakingSystem : SystemBase`。与 repo 整体 39 个 ISystem 迁移方向不一致。Baking system 需持有 managed `Snapshot` property 是合理理由，但应在文档中说明选择理由（JOB-01 要求）。
