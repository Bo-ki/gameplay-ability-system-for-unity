# 09 Burst 编译、向量化与 AOT

## 职责

本主题维护 Burst 在 Runtime Core 热路径、Player 验收、FunctionPointer、vectorization 和 AOT 配置中的使用规则。
...
```csharp
// 定义函数指针类型
public struct ExecutionCalculationFunction : IFunctionPointer
{
    public delegate float Delegate(ref ExecutionContext ctx);
}
...
Burst 的自动向量化需要编译器能证明 code 无别名冲突：

```csharp
// 使用 [NoAlias] 标记无别名关系，帮助编译器向量化
[BurstCompile]
...
| 证据 | 结论 |
|---|---|
| `burst/building-aot-settings.md` | Player/AOT 有独立 Burst 设置 |
| `burst/csharp-function-pointers.md` | FunctionPointer 标量调用可能失去向量化；批处理 job 优先 |
| `burst/optimization-loop-vectorization.md` | loop vectorization 需要可分析循环和明确数据布局 |
| `burst/aliasing.md` | aliasing 影响 Burst 优化和正确性 |

## 使用模式与反模式

**正确模式：**
- Hot path system/job → `[BurstCompile]` + unmanaged 数据
- 动态算法选择 → FunctionPointer + 批处理粒度
- Player 报告 → 包含 AOT/Safety/架构/warmup 信息
- Debugger → 区分 Burst warmup 帧与稳定帧

**反模式：**
- 把 FunctionPointer 当 OOP 虚方法链替代品（逐 entity invoke）
- Managed component 替代 Blob/struct config
- 用 Burst 通过率证明架构正确（不覆盖 query/allocator/structure 问题）

## EX-GAS 项目解读

### 对 Ability/Effect 逻辑的约束

旧 `AbilityLogicBase` / `MMC class` / `虚函数激活链` 方向必须放弃。目标态替换路径：
...
### FunctionPointer 在 ExecutionCalculation 中的角色
...
2. **Editor Burst 通过 ≠ Player Burst 通过**：AOT 平台的限制更多
