...
| bridge/direct-EM | `Integration/GasCore` facade + runtime host / catalog session / observation gateway / lifecycle / ticker / contracts | GAS init/tick/catalog/ASC lifecycle 已集中到 adapter；当前 direct EM 分布在 host、observation、lifecycle、catalog 和 ticker owner | assembly 内直接 `EntityManager` / raw identity matcher 是集中后的风险，不是目标态完成 |
...
8. `AutoChessGasBattleReportFactProjector` 消费 runtime structured log，通过 `AutoChessRuntimeUnitResolver` 把 Core fact 转成 AutoChess battle facts；raw ASC `Entity` 仍停留在 Integration/GasCore 的投影边界内，但当前匹配方式已收缩为 `ASCHandle.MatchesRuntimeEntity(...)`，不再由 lifecycle 显式导出 raw ASC entity。
...
13. 2026-06-07 batchmode 验收已把 active mutation 旧 carrier 退出变成机器门：稳定摘录见 `_归档/2026-06-07-GAS架构瘦身执行记录.md` 的 `追加验收：ActiveMutation owner-local scratch`，原始易失日志路径为 `Temp/AutoChessBattleValidation-ActiveMutationOwnerLocal-Run2.log`；`activeMutationCommands=200`、`activeMutationOwnerGroups=200`、`activeMutationMigrationCarriers=0`，`AutoChessGeneratedScenarioProfile.MaxActiveMutationMigrationCarriers=0`，且 validation result 仍为 `passed=True` / `thresholdsPassed=True` / `runtimeChainPassed=True`。当前仍不能把 ActiveMutation 写成 scale-ready，因为 `activeMutationEstimatedRandomLookups=3000` 仍存在。
...
| observation gateway 负责 official diff、observation reset、diagnostics snapshot 和 timing counter | `Assets/AutoChessDemo/Integration/GasCore/AutoChessGasObservationGateway.cs:11-229` | observation-only / diagnostics |
...
| adapter 对外 handle 已换成 battle unit key，但 Integration/GasCore 内仍能匹配 raw ASC identity | `Assets/AutoChessDemo/Integration/GasCore/AutoChessGasCoreContracts.cs:7-56`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:140-166`、`:218-249`、`AutoChessGasBattleReportFactProjector.cs:7-77` | adapter/raw-identity-matcher |
...
| Runtime Core 统一应用 AutoChess pending delta | `Assets/GAS/Runtime/System/Attribute/GASAttributeModifierDeltaApplySystem.cs:8-158` | runtime-core apply owner；仍是 singleton carrier proof |
...
| raw structured log 到 report fact 的投影位于 Integration/GasCore | `Assets/AutoChessDemo/Integration/GasCore/AutoChessGasBattleReportFactProjector.cs` | boundary projection；仍依赖 runtime resolver 对 structured log raw ASC `Entity` 做匹配 |
...
| observation gateway 通过 `GASRuntimeShell` 取得 singleton 后读写 observation/debug buffer | `Assets/AutoChessDemo/Integration/GasCore/AutoChessGasObservationGateway.cs:18-87`、`:90-229` | observation-only |
| AutoRunner official diff summary 输出 captured / disabled 状态 | `Assets/AutoChessDemo/AutoRunner/AutoChessRuntimeRunner.cs:216-251`、`:259-275` | official-evidence |
...
| active mutation global carrier 已被验收门压成 0 | `EX_GAS_Config/ProjectConfigTable/exgas_config/Datas/AutoChessDemo/autochess.sourcegen.json:30`、`Assets/GAS/Runtime/AbilitySystem/ASCEntityFactory.cs:19-71`、`Assets/GAS/Runtime/System/SystemGroup/GASRuntimeEntityArchetypes.cs:7-501`、`Assets/GAS/Generated/CodeGen/Runtime/RuntimeActiveEffect.gen.cs:339-686`、`00-当前架构事实/_归档/2026-06-07-GAS架构瘦身执行记录.md` | runtime-chain validation evidence；仍有 active mutation random lookup 风险 |
...
| 结构变化 | `AutoChessGasBattleEntityLifecycle` 仍直接创建 driver、切换 driver destroy request、给 ASC 添加 demo component，并维护 battle unit key -> `ASCHandle` registry；`DestroyBattleUnit()` 当前已走 `ASCCommandPort.RequestDestroy()`，不再直接销毁 ASC / granted ability / active effect | Demo 集成层仍有 direct-EM P1 风险，但旧 cleanup direct destroy / granted ability cache 口径已过期 |
...
`AutoChessBattleRuntime` 是 Flow 侧窄入口，`AutoChessGasCoreBridge` 当前已从单体实现变成 Session 侧 facade。真实 direct EM / raw identity matcher 逻辑分布在 `AutoChessGasRuntimeHost`、`AutoChessGasObservationGateway`、`AutoChessGasBattleEntityLifecycle`、`AutoChessGasCatalogSession`、`AutoChessGasRuntimeTicker`、`AutoChessGasBattleReportFactProjector` 和 `AutoChessGasCoreContracts`。这是正向集中：业务层不再散落调用 `GASManager.EntityManager`，但 `GASRuntimeShell` internal capability 仍会把 `World`、`EntityManager`、runtime singleton 和 job drain 能力交给 adapter implementation，`Integration/GasCore` 内部仍直接读写 ECS 并消费 structured log raw ASC key：

- `CreateBattleDriver()` 直接 `CreateEntity()` / `AddComponentData()`
- `AutoChessGasBattleUnitHandle` 当前只持有 battle unit key；旧 unit-handle raw-entity 导出口已无命中，但 lifecycle registry 仍保存内部 `ASCHandle`，report projection 可通过 `ASCHandle.MatchesRuntimeEntity(...)` 匹配 structured log raw ASC entity
...
- `BattleUnitRegistry` 以 battle unit key 缓存 `ASCHandle`，report projection / live read 仍会在 Integration/GasCore 内部使用 raw ASC identity matcher
...
| `CreateBattleDriver()` / `DestroyBattleDriver()` | `AutoChessGasCoreBridge.cs:12-32`、`AutoChessGasBattleEntityLifecycle.cs:46-104` | 结构变化风险 | 迁到 request/structural commit owner，或证明 driver 是 session-owned bootstrap entity |
| `CreateBattleUnit()` / `DestroyBattleUnit()` | `AutoChessGasCoreBridge.cs:7-9`、`:35-43`、`AutoChessGasBattleEntityLifecycle.cs:10-43`、`:106-132`、`:140-180` | unit lifecycle adapter | unit create 仍经 Shell 拿 `EntityManager` 并写 demo component；unit destroy 已走 ASC destroy request，需要保留防回流 gate |
| `BattleUnitRegistry` / runtime unit resolver | `AutoChessGasBattleEntityLifecycle.cs:11-12`、`:67-80`、`:140-166`、`:218-249`、`AutoChessGasCoreContracts.cs:7-56` | battle unit key registry / raw ASC projection matcher | 对外 key 已收缩为 battle unit key；Integration/GasCore 内 raw ASC matcher 仍不能作为业务稳定 key，后续改为 report key / structured log key 或 Boundary snapshot |
...
| `CreateOfficialToolDiffSummary()` / `HasRequiredRuntimeChain()` | `AutoChessRuntimeRunner.cs:216-251`、`:259-275` | official-evidence | captured / disabled / unsupported 必须和性能口径分开 |
...
| Battle unit registry / runtime unit resolver | `AutoChessGasCoreBridge.cs:17-22`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:140-166`、`:218-249`、`AutoChessGasCoreContracts.cs:7-56`、`AutoChessGasBattleReportFactProjector.cs:7-77` | battle unit key registry / report fact projection | resolver 继续把 structured log ASC `Entity` 映射到 battle unit index，但映射方式已变为 `ASCHandle.MatchesRuntimeEntity(...)` 并收缩到 Integration/GasCore 投影边界；旧 ability cache 口径已过期 | 只能作为 migration key，后续改 report key / structured log key |
...
| Live attribute read | `AutoChessGasCoreBridge.cs:40-43`、`AutoChessGasBattleEntityLifecycle.cs:120-132`、`AutoChessBattleSession.cs:183-186` | live read model | 业务胜负 / result 刷新仍经 `ASCReadModel(em, ASCHandle)` 读取 ECS snapshot | R1/R6 要改为 BoundaryProjection snapshot / report key |
...
raw ASC identity 投影面仍未退出：

| 传播点 | 代码证据 | 当前用途 | 风险 |
|---|---|---|---|
| `BattleUnitRegistry` / `ASCHandle` | `AutoChessGasBattleEntityLifecycle.cs:11-12`、`:140-166` | 以 battle unit key 找回内部 `ASCHandle` | raw ECS identity 已不通过 unit handle public 方法导出，但 adapter 内部仍保留可匹配 runtime entity 的 handle |
| `AutoChessRuntimeUnitResolver` / `AutoChessRuntimeUnitLink.Matches(Entity)` | `AutoChessGasBattleEntityLifecycle.cs:218-249` | 以 `ASCHandle.MatchesRuntimeEntity(ascEntity)` 解析 battle unit index | Integration/GasCore 的 report fact projection 仍依赖 structured log raw ECS identity |
| session runtime unit resolver input | `AutoChessBattleSession.cs:113-120`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:218-249` | session 传入 unit handle，Integration/GasCore 内部转成 `ASCHandle` link | raw key 已从 `Battle/Report` 收缩到 adapter projector，但仍需替换为 report key / structured log key |
| alive / attribute refresh | `AutoChessBattleSession.cs:184-189` | alive 判断和 Health / Energy 读取 | 业务胜负判断继续由 adapter live-read ECS |
| report fact projector key | `AutoChessGasBattleReportFactProjector.cs:7-77`、`AutoChessGasCoreContracts.cs:44-56`、`:111-131` | structured log 中 `SourceAsc` / `TargetAsc` 经 resolver 转成 battle unit index；`AutoChessBattleReportBuilder.cs` 只消费 `AutoChessBattleReportFact[]` | raw ECS key 已缩到 Integration/GasCore，但后续仍应改为 report key / structured log key / opaque handle |
...
这比旧状态更接近目标态：AutoChess extension 不再是第二套 Attribute Apply owner。但当前承载仍是 singleton `AttributeModifierBuffer` `MigrationProofOnly`，apply 仍通过 `BufferLookup<AttributeValueBuffer>` 随机写目标 ASC；它不能被写成 scale-ready 终局。后续 R3 仍需要把 execution output / attribute delta 推向 `NativeStream` deterministic merge 或 target-grouped owner-local range，并输出 pending/applied/skipped、target group、random lookup、buffer pressure 和 fact patch counter。
