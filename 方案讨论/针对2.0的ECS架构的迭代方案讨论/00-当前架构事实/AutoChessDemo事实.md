...
| adapter 对外 handle 已换成 battle unit key，report 投影已转 report key | `Assets/AutoChessDemo/Integration/GasCore/AutoChessGasCoreContracts.cs:7-58`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:140-166`、`:211-251`、`AutoChessGasBattleReportFactProjector.cs:7-77`、`Assets/GAS/Runtime/Event/GasStructuredLogExport.cs:419-473` | adapter/report-key-projection；剩余 raw identity 风险在 registry / live read / driver handle |
...
| Runtime Core 统一应用 AutoChess pending delta | `Assets/GAS/Runtime/System/Attribute/GASAttributeModifierDeltaApplySystem.cs:8-195` | runtime-core apply owner；owner-local path 已是 ASC chunk-local `IJobChunk`，剩余风险是 singleton stream carrier 和 stream migration fallback |
...
- `BattleUnitRegistry` 以 battle unit key 缓存 `ASCHandle`，live read 仍会在 Integration/GasCore 内部使用 `ASCHandle` 捕获 ECS snapshot；report projection 已转 report key coverage，但 report key 解析仍依赖 Boundary export 读取 ASC 上的 `ASCBoundaryReportKeyComponent`
...
| `BattleUnitRegistry` / runtime unit resolver | `AutoChessGasBattleEntityLifecycle.cs:11-12`、`:67-80`、`:140-166`、`:211-251`、`AutoChessGasCoreContracts.cs:7-58`、`GasStructuredLogExport.cs:419-473` | battle unit key registry / report key projection | 对外 key 已收缩为 battle unit key；report projection 已消费 report key，剩余风险是 registry/live read 仍通过内部 `ASCHandle` 触达 ECS，后续需要 Boundary snapshot / stable report key coverage gate |
...
| Battle unit registry / runtime unit resolver | `AutoChessGasCoreBridge.cs:17-22`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:140-166`、`:211-251`、`AutoChessGasCoreContracts.cs:7-58`、`AutoChessGasBattleReportFactProjector.cs:7-77`、`GasStructuredLogExport.cs:419-473` | battle unit key registry / report fact projection | resolver 继续把 structured log report key 映射到 battle unit index；旧 raw ASC matcher 与 ability cache 口径已过期 | 固化 report key coverage，live read / registry raw handle 退出转 R1/R6 |
...
| 传播点 | 代码证据 | 当前用途 | 风险 |
|---|---|---|---|
| `BattleUnitRegistry` / `ASCHandle` | `AutoChessGasBattleEntityLifecycle.cs:11-12`、`:140-166` | 以 battle unit key 找回内部 `ASCHandle` | raw ECS identity 已不通过 unit handle public 方法导出，但 adapter 内部仍保留 command/read compatibility handle |
...
| session runtime unit resolver input | `AutoChessBattleSession.cs:113-120`、`AutoChessGasBattleEntityLifecycle.cs:67-80`、`:211-251` | session 传入 unit handle，Integration/GasCore 内部转成 report-key link | report key 已从 `Battle/Report` builder 上游统一生成；仍需保证 Boundary snapshot / report key coverage 不缺失 |
| alive / attribute refresh | `AutoChessBattleSession.cs:184-189` | alive 判断和 Health / Energy 读取 | 业务胜负判断继续由 adapter live-read ECS |
| report fact projector key | `AutoChessGasBattleReportFactProjector.cs:7-77`、`AutoChessGasCoreContracts.cs:46-58`、`:113-133`、`GasStructuredLogView.cs:43-44`、`GasStructuredLogExport.cs:419-473` | structured log 中 `SourceReportKey` / `TargetReportKey` 经 resolver 转成 battle unit index；`AutoChessBattleReportBuilder.cs` 只消费 `AutoChessBattleReportFact[]` | report projector raw key 已缓解；后续重点是 coverage gate、live read snapshot 和 driver raw entity 退出 |
