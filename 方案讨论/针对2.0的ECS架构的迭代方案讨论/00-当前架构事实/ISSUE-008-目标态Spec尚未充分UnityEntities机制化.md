...
1. `SystemAPI.Query` 主线程 foreach 在 `Assets/GAS/Runtime` 与 generated runtime 当前 0 命中；后续不足不是数量，而是必须防止它重新进入 Core hot path，并继续把 Cue managed stored query、Debugger gather、GlobalTimer registered/cache owner 与 ActiveEffectStore registered owner cache 分 owner 计数。
