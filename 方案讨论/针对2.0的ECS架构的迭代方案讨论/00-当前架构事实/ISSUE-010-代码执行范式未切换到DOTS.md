...
- Runtime + generated runtime 当前有 31 个 `ISystem`。
...
- generated `GASActiveEffectMutationApplySystem` 已调度 `[BurstCompile] GEActiveEffectMutationGatherJob : IJob` + `[BurstCompile] GEActiveEffectMutationChunkApplyJob : IJobChunk`；旧 public/static `TryApplyActiveMutation(EntityManager, ...)` helper 与旧 serial `GEActiveEffectMutationApplyJob : IJob` 已退场。
...
5. 2026-06-07 Run3 AutoChess runner 能跑通且 `debugErrors=0` / `blockingDebugErrors=0`，但同轮 `syncQueryBudget=13`、`dependencyWaitRisks=4`、`requiredStructuralPlaybacks=5`、`recordedStructuralPlaybacks=0` 暴露 query / dependency / structural evidence 仍未闭合，不能把“能跑”写成结构变化已合规。
