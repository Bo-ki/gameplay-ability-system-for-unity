...
| `EffectCommandSpecStream` singleton writer / frame counter | `Effect/Component/Dynamic/GEEffectCommandSpecStream.cs:279-967`，其中 `BeginCommandWriter` / `BeginOwnerLocalFactWriter` / `PrepareFrameLocalData` / pressure counters 仍围绕 `GEEffectCommandStreamComponent` singleton owner | singleton fan-in proof carrier / owner-local fact export bridge | 显式 writer、owner-local fact writer 和 pressure evidence 是正向事实；但 command / set-by-caller 与 stream export 仍不能当作 scale-ready `NativeStream` fan-in |
...
| generated ECB create / destroy | `rg` 当前未在 `Assets/GAS/Generated/CodeGen/Runtime` 命中 `EntityCommandBuffer`、`ISystem`、`OnUpdate`、`ComponentLookup` 或 `BufferLookup`；`RuntimeActiveEffect.gen.cs` 当前只是 12 行 `RuntimePureGlue` marker | generated lifecycle 已退出当前磁盘面 | active-effect 主 owner 已迁入手写 Runtime Core；后续 R5 重点变为 generated lifecycle / ECB 回流阻断、report / manifest / file / schedule 对账和 catalog lifetime / dispose owner 证据 |
...
R0-R8 叶子任务拆分后，重新对 `Assets/GAS/Runtime`、`Assets/GAS/Generated/CodeGen/Runtime` 和 `Assets/AutoChessDemo` 执行 `rg` 静态复核。本截面形成时 `codedb_status` 曾不可用，报 `failed to load project: 系统找不到指定的文件。 (os error 2)`，因此该截面的 API 分类采用文本扫描和定点读取作为证据；本轮收口复测 `codedb_status` 已恢复 ready，当前为 434 files / 434 outlines / 766 chunks / graph 448 nodes / 1831 edges / 34 communities / scan ready，后续代码截面可优先恢复 codedb + `rg` 交叉复核。
...
4. SourceGenerator gate 截面已改为更收窄的当前报告：`GeneratedRuntimeBoundaryHits=0`、`GeneratedRuntimePureGlueArtifacts=4`、`GeneratedRuntimeLifecycleMigrationArtifacts=0`、`GeneratedRuntimeLifecycleHits=0`、`GeneratedRuntimeStructuralChangeHits=0`、`GeneratedRuntimeOwnershipHits=0`、`GeneratedRuntimeRandomWriteLookupHits=0`、`GeneratedRuntimeManagedConfigHits=0`、`GeneratedRuntimeBoundaryGateMode=blocking-unclassified-lifecycle-migration`、`GeneratedRuntimeUnclassifiedBoundaryHits=0`。这说明 generated lifecycle artifact 已退出 manifest，未分类回流仍被阻断；但 report / manifest / file / schedule 对账、system budget、catalog lifetime / dispose owner 证据和负例验证仍必须走 R5/R7 收口，不能写成 SourceGenerator pure glue 完成证明。

### codedb 恢复后的整体项目审查补充（2026-06-07）

本截面使用 `codedb_status`（434 files / 434 outlines / 766 chunks / graph 448 nodes / 1831 edges / 34 communities / scan ready）和 `rg` 交叉复核。它只记录当前代码事实；目标态约束写入 `../../01-目标态架构共识/90-目标态不变量.md`。
...
6. generated / handwritten runtime 有正向 job / Blob 进展：`RuntimeAbilityActivation.gen.cs` 与 `RuntimeEffectInstant.gen.cs` 当前均为 12 行 marker；ability activation owner 已转入手写 `AbilityCommitSystem` / `GASRuntimeDefinitionResolver`；instant spec / reduce owner 已转入手写 `GEEffectInstantSystems.cs`，其中 spec collect 使用 `IJobChunk`，build/reduce 仍需要按 lookup owner 审查；active-effect pre-tick / remove / mutation apply 已由手写 `GEActiveEffectLifecycleSystems.cs` 调度 `GASActiveEffectRuntime` helper；`DefinitionCatalog.gen.cs` 是 sorted code -> index lookup，并用 `BlobBuilder(Allocator.Temp)` 构建 runtime catalog blob。
7. generated runtime 仍不是 release-ready 完成态：`RuntimeActiveEffect.gen.cs` 当前只是 `RuntimePureGlue` marker；active mutation command normalize、helper/job/snapshot/mutation/tick/remove 主体已迁到手写 `GEActiveEffectCommandNormalizeSystem.cs`、`GASActiveEffectRuntime.cs` 和 `GEActiveEffectLifecycleSystems.cs`；`ActiveEffectLifecycleOwnerSystems.cs` 当前磁盘缺失。active mutation 和 pending AttributeDelta owner-local apply 的 lookup 面积已经大幅收窄，pending AttributeDelta stream migration fallback 已删除，但 command carrier、active effect SourceAttribute snapshot lane、capacity / spill、generated lifecycle 防回流、catalog bootstrap owner 和 release-ready gate 仍未达到 pure glue / store-only 终局。
8. 生成报告当前显示 `RuntimeForbiddenDependencyHits=0`、`GeneratedRuntimeBoundaryHits=0`、`GeneratedRuntimePureGlueArtifacts=4`、`GeneratedRuntimeLifecycleMigrationArtifacts=0`、`GeneratedRuntimeLifecycleHits=0`、`GeneratedRuntimeSystemRegistrationHits=0`、`GeneratedRuntimeStructuralChangeHits=0`、`GeneratedRuntimeOwnershipHits=0`、`GeneratedRuntimeRandomWriteLookupHits=0`、`GeneratedRuntimeManagedConfigHits=0`、`GeneratedRuntimeBoundaryGateMode=blocking-unclassified-lifecycle-migration`、`GeneratedRuntimeUnclassifiedBoundaryHits=0`。因此当前状态是“generated lifecycle artifact 和 companion 文件面已退出，report boundary 分项归零，但 system budget、负例验证、manifest/report/file/schedule 对账和防回流 gate 未完成 release-ready 对账”，不是 SourceGenerator pure glue 完成证明。
