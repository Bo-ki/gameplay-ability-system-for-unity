...
2. generated runtime 已接入 `GASDefinitionCatalogBlob`；ability commit/auto-end、explicit remove pending 已使用 chunk `EnabledMask`，ability lifecycle cancel/end/destroy-on-cleanup 已通过 frame-local request buffer 归并到 ability chunk applicator，ASC dirty / active modifier present 已通过 frame-local `AttributeOwnerMarkerRequestBuffer` 归并到 ASC chunk applicator，execution output applied 已由 effect chunk applicator 写入。spec-build/attribute reduce 已是 `[BurstCompile] IJob`，normalize/active mutation/active pre-tick/active remove 已改为 scheduled `IJob` / `IJobChunk`，但 active mutation 仍是 singleton serial job 并依赖 Buffer/ComponentLookup random access。
...
| `QRY-01` | Core `Complete()` 与 active pre-tick / attribute recalc / ASC command resolve / generated ability commit / active remove / fact projection 主线程 Query 或 buffer loop 已退场；Cue managed boundary 当前是 stored query + `ToEntityArray` materialization，不是 `SystemAPI.Query` foreach；generated active mutation singleton serial job / lookup random access 仍需治理 | 部分满足 |
...
| `BUR-01` | generated instant spec-build / attribute-reduce / active mutation 已输出 `[BurstCompile] IJob`；active mutation 仍是单 owner serial job，不可写成 hot path 终局 | 部分满足 |
...
2. **旧文档中的 “22 个 ToEntityArray 热路径” 不再成立。** 当前 `ToEntityArray` 精确命中 5 处：Debugger observation、Cue managed boundary 和 GlobalTimer singleton fallback；`ActiveEffectStore` 当前是 `CreateEntityQuery + CalculateEntityCount + GetSingletonEntity` global index fallback，不再是 `ToEntityArray` 命中。`Complete()` 当前已清零，`SystemAPI.Query<...>` / `SystemAPI.Query(...)` 在 Runtime + generated runtime 当前 0 命中。generated ability commit/current ability auto-end 的 random enableable、instant spec/reduce 的未 Burst 主线程诊断、active mutation 旧 `EntityManager` helper、explicit remove pending lookup、ability lifecycle cross-entity marker 写入、ASC dirty/present random enableable、execution output applied random enableable 和 ability seed scratch 均已退场。真正需要继续关注的是 generated active mutation singleton serial job、Buffer/ComponentLookup random access、全局 singleton stream、GlobalTimer / ActiveEffectStore singleton fallback owner 化和结构变化证据缺口。
