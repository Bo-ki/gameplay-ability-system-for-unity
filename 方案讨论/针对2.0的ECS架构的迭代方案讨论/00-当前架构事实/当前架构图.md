...
> 上次更新：2026-06-08 | 事实源：`Assets/GAS/Runtime` + `Assets/GAS/Generated/CodeGen/Runtime` 当前代码
...
| 规则 | 当前状态 | 结论 |
|---|---|---|
| `SYS-01` | 权威状态主要在 ECS，但 `ASCCommandPort`、hand-written Runtime consumers、config/prototype 路径仍有 `EntityManager` / capability owner 需要分类 | 部分满足 |
| `SYS-02` | 5 个物理 phase group 已真实注册到 FixedStep | 满足当前骨架 |
| `SYS-03` | 物理 group 数量已收敛；hand-written runtime system 数量、generated lifecycle 防回流和 lookup 刷新仍需计数 | 持续监控 |
...
| `BUR-01` | hand-written instant spec-build / attribute-reduce、active mutation normalize 与 active mutation gather / chunk apply 已输出 `[BurstCompile]` job；active mutation command carrier 仍非 hot path 终局 | 部分满足 |
...
| generated runtime registration 防回流 | guard-only | `Assets/GAS/Runtime/System/SystemGroup/GASSystemScheduleContract.cs:230-256` 的 `GeneratedCommandResolveSystemTypeNames` / `GeneratedCoreSimulationSystemTypeNames` 当前为空数组，`AddSystemsByTypeName(...)` 缺失 type 会 fail-fast | 不能再写成当前通过 generated type-name 列表注册主链；generated catalog / pure glue 仍需审查，generated lifecycle 回流默认阻断 |
...
2. **旧文档中的 “22 个 ToEntityArray 热路径” 不再成立。** 当前 `EntityManager.CreateEntityQuery` Runtime 可执行命中 0 处；当前 `ToEntityArray` 运行命中 3 处：Debugger observation 和 Cue managed boundary。`GasRuntimeDebugger` singleton lookup、`GASRuntimeFrameContext` current-frame lookup 与 `ActiveEffectStore` global index owner 均已改为 registered/cache owner，不再创建 fallback query；Debugger observation materialization 已进入专用 counter 和 AutoChess evidence。`Complete()` 当前已清零，`SystemAPI.Query<...>` / `SystemAPI.Query(...)` 在 Runtime + generated runtime 当前 0 命中。ability commit/current ability auto-end 的 random enableable、instant spec/reduce 的未 Burst 主线程诊断、active mutation 旧 `EntityManager` helper、旧 active mutation serial apply、explicit remove pending lookup、ability lifecycle cross-entity marker 写入、ASC dirty/present random enableable、execution output applied random enableable、pending AttributeDelta owner materialization、pending AttributeDelta stream migration fallback、GlobalTimer entity array/fallback query 和 ability seed scratch 均已退场。真正需要继续关注的是 active mutation singleton command carrier / SourceAttribute snapshot lane、全局 singleton stream、instant delta record / fan-in 证明、ActiveEffectStore capacity/cache integrity 证据、Debugger observation 阈值、generated lifecycle 防回流和结构变化证据缺口。
3. **generated catalog / pure glue 是当前事实的一部分，但 generated type-name registry 不是当前主调度来源。** 不能只审查 handwritten Runtime 的消费者，也不能把旧“1 个 CommandResolve + 6 个 CoreSimulation generated systems”写成当前事实；`GASSystemScheduleContract` 当前 generated type-name 数组为空，缺失 generated type fail-fast 只作为防回流机制保留，`RuntimeSystemRegistration.gen.cs` 已退场。
