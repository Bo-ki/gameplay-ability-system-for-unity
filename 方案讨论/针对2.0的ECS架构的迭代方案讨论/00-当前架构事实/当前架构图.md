...
| `QRY-01` | Core `Complete()` 与 active pre-tick / attribute recalc / ASC command resolve / generated ability commit / active remove / fact projection 主线程 Query 或 buffer loop 已退场；Cue managed boundary 仍用 `SystemAPI.Query`，generated active mutation singleton serial job / lookup random access 仍需治理 | 部分满足 |
