...
2. **旧文档中的 “22 个 ToEntityArray 热路径” 不再成立。** 当前 `ToEntityArray` 运行命中 5 处：Debugger observation、Cue managed boundary 和 GlobalTimer singleton fallback；`ActiveEffectStore` 当前是 `CreateEntityQuery + CalculateEntityCount + GetSingletonEntity` global index fallback，不再是 `ToEntityArray` 命中。`Complete()` 当前已清零，`SystemAPI.Query<...>` / `SystemAPI.Query(...)` 在 Runtime + generated runtime 当前 0 命中。generated ability commit/current ability auto-end 的 random enableable、instant spec/reduce 的未 Burst 主线程诊断、active mutation 旧 `EntityManager` helper、旧 active mutation serial apply、explicit remove pending lookup、ability lifecycle cross-entity marker 写入、ASC dirty/present random enableable、execution output applied random enableable、pending AttributeDelta owner materialization 和 ability seed scratch 均已退场。真正需要继续关注的是 active mutation singleton command carrier / SourceAttribute snapshot lane、全局 singleton stream、stream migration fallback、GlobalTimer / ActiveEffectStore singleton fallback owner 化和结构变化证据缺口。
