...
20. 2026-06-08 第一轮 Debugger 改造已把 AutoChess headless logic budget 的数据导向指标前移到 Runtime Debugger 模块：新增 `GasRuntimeDataOrientedScorecard`，由 AutoChess validation 创建 scorecard 后再做预算阈值判定，summary 显式输出 `scorecardSource=GasRuntimeDataOrientedScorecard`。第二刀已在 `GasRuntimeDebugger` 增加 Runtime-owned scorecard text export，并让 AutoChess runner / report 调用 `GasRuntimeDebugger.ExportDataOrientedScorecardToText(...)` 输出 `runtimeDataOrientedScorecard`。这只是 Runtime-owned scorecard 与 export 起点，尚未拆除 `GasRuntimeDebugger.cs` 的单体事件结构、稀疏大事件 row 和 diagnostic materialization 链。
...
| ISSUE-003 | Debugger/Official diff 已有工具；Observation materialization、Magnitude Source evidence 和 strict budget scorecard 已进入证据链，但 `GasRuntimeDebugger` 当前仍是 diagnostics proof 单体，稀疏大事件结构、materialization、official diff 和 derived export 需要拆成数据导向 Debugger owner | Active | P1 | R4 / R8 |
