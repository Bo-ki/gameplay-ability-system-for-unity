...
| ISSUE-003 | Debugger/Official diff 已有工具；Observation materialization、Magnitude Source evidence、strict budget scorecard、`GasRuntimeMetricFamilySnapshot`、`GasRuntimeDiagnosticEvidenceSnapshot`、`GasRuntimeDiagnosticRetentionPolicy` 与 `GasRuntimeDerivedExportSink` 已进入证据链，AutoChess 外部消费面已改走 `RuntimeDiagnostics.Evidence.*`。DebuggerProbe Run3 已证明 scorecard 可混合 performance timing、diagnostic GAS/data/API family 与 performance overhead gate，并能定位 RW / owner-local range / execution fan-out / materialization 热点；但 `GASRuntimeDebuggerComponent` 仍混装 config/state/counter，稀疏大事件结构、metric family buffer、materialization 和 official diff 仍需拆成数据导向 Debugger owner | Active | P1 | R4 / R8 |
...
| ISSUE-014 | AutoChess x50 headless 业务链路和 strict pure logic budget 在 DebuggerProbe Run3 均通过，但 `performanceExcellentPassed=False`，原因是 Profiler evidence disabled，且 Debugger/Journaling 仍定位出 RW、owner-local range、execution fan-out 和 diagnostic materialization 热点；后续必须补 x100/x1000、Profiler enabled 和数据形态瘦身 | Active | P1 | R4 / R8 / CoreSimulation 瘦身 |
