...
12. 2026-06-08 本轮 codedb 当前项目截面为 430 files / 430 outlines / 771 chunks / graph 444 nodes / 1786 edges / 32 communities / scan ready；Runtime path-prefix module-map 主社群为 141 files / 1990 indexed symbols，dependency edges 为 internal 530、boundary 175、incoming 149、outgoing 26。`ActiveEffectLifecycleOwnerSystems.cs` 已退出 SourceGenerator 输出、manifest 和 generated boundary report，但仍作为 generated runtime 物理 asmdef 内的手写 companion owner 保留；`RuntimeActiveEffect.gen.cs` 当前主要承载 generated active effect helper / job / lookup / ECB / NativeContainer owner。该拆分降低 generated boundary hit 面，不改变“剩余 generated lifecycle 仍为 MigrationProofOnly、SourceGenerator 目标态只能保留 immutable catalog / lookup / pure glue / validation”的事实判定。
13. `OwnerLocalGameplayFactBuffer` 已纳入 ASC archetype、初始化容量和完整性检查；当前写入面已覆盖 pending AttributeDelta chunk-local apply、generated instant reduce 和 execution output modifier apply 产生的 Attribute fact；`GameplayOwnerLocalFactFramePrepareSystem` 在 FramePrepare 清理 ASC 本地 fact，`GameplayOwnerLocalFactFlushSystem` 在 CoreSimulation 收集后 flush 到现有 `GameplayEventBuffer` stream export，`GameplayFactBoundaryProjectionSystem` 仍从 stream fact 派生 Attribute/Cue/Tag 边界缓冲。该切片是 owner-local fact 的正向局部实现，但不能推出 command/spec/delta/fact 全链路已退出 singleton stream，generated 写入面仍需按 R5/R3/R7 的 `MigrationProofOnly` 退出门继续审查。
...
| ISSUE-010 | 执行范式旧风险已收窄；ASC dirty/present、execution output applied、active mutation apply、pending AttributeDelta owner-local apply 已收口到 owner chunk applicator，execution output typed fact 已进入 NativeStream deterministic merge；当前主要是剩余 singleton stream、SourceAttribute snapshot lane 与 boundary managed query 风险 | Active | P1 | R2 / R3 / R7 |
| ISSUE-011 | 临时 query 泛滥旧口径已缓解；隐式 stream writer/append helper 已收窄为显式 owner 写入，execution output fact 已退出 structural ECB singleton append，active mutation command source 已投影到 ASC owner-local command buffer；当前 API 承载风险集中在剩余 command / set-by-caller / spec / typed fact singleton stream carrier、`GASRuntimeShell` 多能力 facade、global facade 和 generated delta record carrier | Active | P1 | R1 / R2 / R3 / R6 |
...
| 7 个 generated runtime systems | runtime-active | `GASSystemScheduleContract.cs` 的 `GeneratedCommandResolveSystemTypeNames` / `GeneratedCoreSimulationSystemTypeNames` 与 `AddGeneratedRuntimeSystems(...)` | 必须纳入 Runtime 审查范围；`RuntimeSystemRegistration.gen.cs` 当前已退场，手写 registry 对缺失 generated type 已 fail-fast |
