...
6. `ToEntityArray` 当前运行命中 4 处：Debugger observation 3 处、Cue managed boundary 1 处；`GASGlobalTimerSystem` 已优先读取 registered `GASManager.EntityGlobalTimer` owner，cache miss fallback 改为 `CreateEntityQuery + CalculateEntityCount + GetSingletonEntity`，不再 materialize 实体数组。`ActiveEffectStore` 当前也是 `CreateEntityQuery + CalculateEntityCount + GetSingletonEntity` 的 global index fallback，不再是 `ToEntityArray` 命中。当前热路径风险重点已收窄到 singleton stream owner、stream migration fallback、`GASRuntimeShell` internal capability 面、剩余 singleton fallback owner 化和容量/ordering 证据；active mutation apply 与 pending AttributeDelta owner-local apply 均已进入 ASC chunk-local IJobChunk，旧 random lookup 估算为 0。
...
| ISSUE-011 | 临时 query 泛滥旧口径已缓解；当前 API 承载风险集中在 singleton owner、`GASRuntimeShell` 多能力 facade、global facade、stream migration fallback 和 active mutation singleton command carrier | Active | P1 | R1 / R2 / R3 / R6 |
...
- `ToEntityArray()` 当前运行命中为 `GasRuntimeDebugger.cs:889/2260/2458`、`CueManagedLifecycleSystem.cs:39`；Debugger / Cue 为 observation / managed boundary。`GASAttributeModifierDeltaApplySystem` 不再物化 owner entity list；`GASGlobalTimerSystem.cs:125` 和 `ActiveEffectStore.cs:1374-1382` 当前通过 `CreateEntityQuery + CalculateEntityCount + GetSingletonEntity` 做 singleton cache-miss fallback，需要 owner 注册、capacity 和 cache miss 证据，但不能再归入 `ToEntityArray` 命中。
