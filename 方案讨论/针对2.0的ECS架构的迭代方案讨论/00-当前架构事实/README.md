...
20. 2026-06-08 第一轮 Debugger 改造已把 AutoChess headless logic budget 的数据导向指标前移到 Runtime Debugger 模块：新增 `GasRuntimeDataOrientedScorecard`，由 AutoChess validation 创建 scorecard 后再做预算阈值判定，summary 显式输出 `scorecardSource=GasRuntimeDataOrientedScorecard`。第二刀已新增 `GasRuntimeDerivedExportSink`，让 `GasRuntimeDebugger.ExportDataOrientedScorecardToText(...)` 只作为 facade 并输出 Runtime-owned `runtimeDataOrientedScorecard`；AutoChess runner / report 现在还能输出 `metricFamilyMask` 与 `dominantRisk`。第三刀已新增 `GasRuntimeMetricFamilySnapshot` 并挂入 `GasRuntimeDiagnosticSnapshot.MetricFamilies`：scorecard 和 derived export 不再直接依赖 raw counter 拼接，AutoChess summary 新增 `metricFamilySource=GasRuntimeMetricFamilySnapshot`。第四刀已新增 `GasRuntimeDiagnosticEvidenceSnapshot` 并挂入 `GasRuntimeDiagnosticSnapshot.Evidence`：AutoChess validation、hotspot、runtime chain、repeat-run、presentation bridge 和 README 已改为消费 `RuntimeDiagnostics.Evidence.*`，不再直接绑定 `RuntimeDiagnostics.CoreCounters` / `Events` / `Stats` 等易变内部 schema。第五刀已新增 `GasRuntimeDiagnosticRetentionPolicy`，把 event retention 删除策略从 `GasRuntimeDebugger.cs` 单体移出。这只是 Runtime-owned scorecard、family snapshot、evidence envelope、retention policy 与 derived export owner 的起点，尚未拆除 `GASRuntimeDebuggerComponent` 的 config/state/counter 混装、稀疏大事件 row 和 diagnostic materialization 链。
21. 2026-06-08 DebuggerProbe Run3 已完成一轮真实 Unity batchmode 无头 AutoChess 实测：Unity exit code 0，`passed=True`、`headlessLogicBudgetPassed=True`、`performanceExcellentPassed=False`、`blockingDebugErrors=0`。本轮修正了 scorecard 证据源：`metricFamilySource=DiagnosticPassGasData+PerformancePassOverhead`、`metricFamilyMask=0x7F`、`coreFacts=5200`、`activeMutationCommands=200`、`ownerLocalFactMaxOwnerRange=9`、`performanceObservationPollutionRisks=0`。Debugger 定位出的主要热点为 `GetBufferRW=67884`、`GetComponentDataRW=18892`、`EnableComponent=650`、`OwnerLocalGameplayFactBuffer=11250`、`GEEffectCommandStreamComponent=9237`、execution spec scan ratio `3.86:1` 和 diagnostic-only `DebuggerOwner.avgMs=49.094`。这证明 Debugger 已能定位性能热点，但也证明下一轮应按 fact fan-in / active effect pre-tick / instant command prepare / execution spec index / diagnostic materialization owner 拆分继续迭代。
22. 2026-06-08 DebuggerProbe Run4 已新增机器可读 `hotspotAttribution` 矩阵，并由 AutoChess report 与离线分析脚本消费。Run4 仍为 Unity batchmode 真实无头实测：`passed=True`、`headlessLogicBudgetPassed=True`、`performanceExcellentPassed=False`、`avgTickMs=0.926`、`GASTickTotal.avgMs=0.913`、`CoreSimulation.avgMs=0.508`、`BoundaryOwner.avgMs=0.234`、`performanceObservationPollutionRisks=0`、`profilerCaptureState=profiler disabled; Entities profiler modules collect no data`。矩阵把下一轮 owner 锁定为 R3 `OwnerLocalGameplayFactBuffer` dirty span / attribute dirty range、R7/R3 ActiveEffect pre-tick 与 command prepare、R5/R3 execution spec generated index、R4 diagnostic materialization owner 和 R8 Profiler enabled scale gate。离线分析脚本同步新增 `AutoChessProfileBrief.md` 与 `SubReports/*`，默认让 Agent 先读短简报，再按领域读取性能、热点、Journaling、Debugger 或战斗日志子报告。该证据证明 Debugger 已能做任务路由，不证明性能优秀或 Profiler 闭环完成。
23. 2026-06-08 DebuggerProbe Run10c 已完成 prepare dirty owner index 切片：AutoChess x50 `passed=True`、`headlessLogicBudgetPassed=True`、`repeatRunPassed=True`、`avgTickMs=0.898ms`、`CoreSimulation.avgMs=0.494ms`、FramePrepare `avgMs=0.037ms`。相对 Run9b，instant prepare `2400/1450/950 -> 950/0/950`，active mutation prepare `2400/1750/650 -> 200/0/200`，`GetBufferRW=60684 -> 48852`，`EnableComponent=650` 未膨胀。该证据证明 dirty owner index 能替代全 ASC 稀疏 owner 扫描；但 `performanceExcellentPassed=False` 仍由 Profiler disabled 阻塞，且 `OwnerLocalGameplayFactBuffer`、`GEEffectCommandStreamComponent`、ActiveEffect pre-tick、execution spec scan 和 diagnostic materialization 仍是下一轮 owner。
...
| ISSUE-003 | Debugger/Official diff 已有工具；Observation materialization、Magnitude Source evidence、strict budget scorecard、`GasRuntimeMetricFamilySnapshot`、`GasRuntimeDiagnosticEvidenceSnapshot`、`GasRuntimeDiagnosticRetentionPolicy` 与 `GasRuntimeDerivedExportSink` 已进入证据链，AutoChess 外部消费面已改走 `RuntimeDiagnostics.Evidence.*`。DebuggerProbe Run10c 已证明 frame-lane counter 能驱动 dirty owner index 优化：instant / active mutation prepare 稀疏 owner 扫描明显下降；但 `GASRuntimeDebuggerComponent` 仍混装 config/state/counter，稀疏大事件结构、metric family buffer、materialization 和 official diff 仍需拆成数据导向 Debugger owner | Active | P1 | R4 / R8 |
...
| ISSUE-014 | AutoChess x50 headless 业务链路和 strict pure logic budget 在 DebuggerProbe Run10c 通过，prepare dirty owner index 已让 instant prepare `2400/1450/950 -> 950/0/950`、active mutation prepare `2400/1750/650 -> 200/0/200`；但 `performanceExcellentPassed=False`，原因是 Profiler evidence disabled，且 hotspot matrix 仍定位出 fact fan-in、singleton stream RW、active effect pre-tick、execution fan-out 和 diagnostic materialization 热点；后续必须补 x100/x1000、Profiler enabled 和数据形态瘦身 | Active | P1 | R3 / R4 / R5 / R7 / R8 |
