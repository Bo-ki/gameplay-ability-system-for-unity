...
| ISSUE-003 | Debugger/Official diff 已有工具；Observation materialization、Magnitude Source evidence、strict budget scorecard 与 `GasRuntimeDerivedExportSink` 已进入证据链，但 `GasRuntimeDebugger` 当前仍是 diagnostics proof 单体，稀疏大事件结构、metric family buffer、materialization 和 official diff 仍需拆成数据导向 Debugger owner | Active | P1 | R4 / R8 |
