...
| `components-buffer-jobs.md`：`BufferLookup` 是 job 内随机访问 buffer 的工具，不等同于 scale-ready store | active mutation apply 与 pending AttributeDelta owner-local apply 已进入 ASC chunk-local path；仍需把 singleton stream carrier、stream migration fallback 和跨 owner snapshot lane 继续作为 lifecycle store 风险 |
...
5. pending AttributeDelta 与 active/pre-tick 的 `BufferLookup` / `ComponentLookup` 写入被 owner-local store、target-grouped merge、snapshot lane 或可量化 profile 证明覆盖。
