...
3. 继续拆 pre-tick / active effect slot tick 的 `SourceAttribute` magnitude snapshot lane；execution calculation / managed resolver 当前已有 capture miss、fallback 和 lookup evidence，但仍不能把 evidence 显性化写成 snapshot lane 终局。
