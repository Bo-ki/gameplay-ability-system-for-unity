...
1. `Assets/GAS/Runtime` + `Assets/GAS/Generated/CodeGen/Runtime` 当前共检出 30 个 `ISystem` 类型。
...
6. `GASActiveEffectMutationApplySystem` 当前调度 `GASGeneratedActiveEffectRuntime.GEActiveEffectMutationApplyJob : IJob`；旧 public/static `TryApplyActiveMutation(EntityManager, ...)` 已退场。当前 active mutation command 会先复制到 frame-local `NativeList`，再按 owner/sequence/context 排序，并按 owner range 批处理，写入 mutation 数、owner group 数、最大 owner range、排序移动数。mutation 结果不再写回 stream owner 的 `ActiveEffectMutationBuffer`，而是写入当前 ASC owner-local `ActiveEffectMutationBuffer` scratch；`ASCEntityFactory` 与 `GASRuntimeEntityArchetypes.ASC` 已把该 buffer 纳入 ASC 核心布局。剩余风险是 active mutation apply 仍是 singleton stream serial job，owner resource 仍通过 helper lookup 抓取，尚未拆成 ASC chunk-local applicator。
...
8. 2026-06-07 x50 batchmode 验收稳定摘录见 `_归档/2026-06-07-GAS架构瘦身执行记录.md` 的 `追加验收：ActiveMutation owner-local scratch`，原始易失日志路径为 `Temp/AutoChessBattleValidation-ActiveMutationOwnerLocal-Run2.log`；关键字段显示 `activeMutationCommands=200`、`activeMutationOwnerGroups=200`、`activeMutationMigrationCarriers=0`、`periodTickDamageFacts=150`，并且 `passed=True` / `thresholdsPassed=True` / `runtimeChainPassed=True`。这说明 active mutation 的旧 global mutation carrier 已退出验收门；但 `activeMutationEstimatedRandomLookups=3000` 仍存在，下一轮必须继续把 singleton serial job / helper lookup random access 拆成 ASC owner chunk-local apply 或 target-grouped applicator。
...
1. `ToEntityArray` 当前精确命中 5 处，不再是旧的 22 处 hot path 诊断。
...
5. Runtime Core 仍存在 generated active mutation singleton stream serial owner、helper BufferLookup/ComponentLookup random access、`GASRuntimeFrameContext` current-frame fallback query 和 `ActiveEffectStore` global index owner fallback query；逐 command store/slot/snapshot lookup 已收束为 owner range 入口。
...
| `ToEntityArray()` | `GasRuntimeDebugger.cs:814/2014/2212` | Debugger / observation-only |
...
| Demo adapter 直接创建/写 ECS / report raw key | `AutoChessBattleRuntime.cs`, `AutoChessGasCoreBridge.cs`, `AutoChessGasBattleEntityLifecycle.cs`, `AutoChessGasRuntimeHost.cs`, `AutoChessGasCatalogSession.cs`, `AutoChessGasRuntimeTicker.cs`, `AutoChessGasBattleReportFactProjector.cs`, `AutoChessGasCoreContracts.cs`, `AutoChessBattleDefinitionCatalogBuilder.cs` | Demo 集中接缝，unit/driver lifecycle、job drain、report projection raw key 需迁入 request/commit/snapshot/report-key owner；generated catalog 安装属初始化路径 |
...
2. 继续拆 `GASActiveEffectMutationApplySystem` 的 singleton serial job / helper lookup random access；当前已补 owner-sorted deterministic apply、owner range applicator、ASC owner-local mutation scratch 与 `activeMutationMigrationCarriers=0` gate，下一步应替换为 ASC chunk-local applicator 或 target-grouped owner-local apply，降低 `activeMutationEstimatedRandomLookups`。
