...
5. Runtime Core 的 pending AttributeDelta owner-local apply 已迁到 ASC chunk-local `IJobChunk`，不再通过 `_ownerDeltaQuery.ToEntityArray(Allocator.TempJob)` 物化 owner，也不再把 owner chunk 写入计为 random lookup；剩余 Runtime Core 风险集中在 singleton stream carrier、stream migration fallback、`GASRuntimeFrameContext` current-frame fallback query 和 `ActiveEffectStore` global index owner fallback query。generated active mutation 逐 command store/slot/snapshot lookup 已收束为 owner range 入口，但 command source 仍来自 singleton carrier。
...
2. active mutation 与 pending AttributeDelta 的旧 random lookup 估算均已退出当前 x50 验收门；后续不应继续围绕这两个字段打补丁，而应把 singleton stream proof 拆成按数据性质选型的 scale-ready carrier，并补 deterministic merge / capacity / spill 证据。
3. 继续拆 active/pre-tick 的 `SourceAttribute` magnitude snapshot lane，避免跨 owner attribute capture 回流到 chunk apply 内。
